...
// ── Row-alignment helper for R8Unorm uploads ─────────────────────────────────
// wgpu requires bytes_per_row to be a multiple of 256. For R8 (1 byte/pixel),
// rows narrower than 256 px need padding.
fn pad_r8_rows(data: &[u8], width: u32, height: u32) -> (Vec<u8>, u32) {
    let aligned = (width + 255) & !255;
    if aligned == width {
        return (data.to_vec(), width);
    }
    let mut out = vec![0u8; (aligned * height) as usize];
    for r in 0..height as usize {
        out[r * aligned as usize..r * aligned as usize + width as usize]
            .copy_from_slice(&data[r * width as usize..(r + 1) * width as usize]);
    }
    (out, aligned)
}
...
impl GlyphAtlas {
    ...
    fn create_texture(device: &wgpu::Device) -> wgpu::Texture {
        device.create_texture(&wgpu::TextureDescriptor {
            ...
            // R8Unorm: 1 byte/pixel for grayscale mask. 16 MiB vs 64 MiB (Rgba8Unorm).
    ...
    /// Upload a rasterized grayscale mask glyph and cache its atlas location.
    ///
    /// `data` must be R8 bytes of size `width × height` (1 byte/pixel).
    /// `is_color` should always be `false` for the mask atlas.
    ...
    pub fn upload(
        ...
        // wgpu requires bytes_per_row to be a multiple of 256 for R8Unorm.
        let (padded, aligned_width) = pad_r8_rows(data, width, height);
        queue.write_texture(
...
// ── ColorAtlas — 1024×1024 Rgba8Unorm for emoji / color glyphs ────────────────

/// GPU atlas for color glyphs (emoji, color bitmap fonts).
/// Smaller than GlyphAtlas (4 MiB vs 16 MiB) since color glyphs are rare.
...
impl ColorAtlas {
    ...
    /// Upload a color RGBA glyph (emoji) and cache its atlas location.
    ///
