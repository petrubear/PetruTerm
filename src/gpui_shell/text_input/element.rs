...
use gpui::{
    ...
    GlobalElementId, LayoutId, PaintQuad, Pixels, ShapedLine, Style, TextRun, UnderlineStyle,
    Window,
};
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), input.placeholder_color.into())
        } else {
            (content, input.text_color.into())
        };
            ...
            (
                ...
                Some(fill(
                    Bounds::new(
                        ...
                        size(gpui::px(2.), bounds.bottom() - bounds.top()),
                    ),
                    input.cursor_color,
                )),
            )
