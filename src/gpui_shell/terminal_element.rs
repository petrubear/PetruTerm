...
use cosmic_text::{
    Attrs, Buffer, Color as CosmicColor, FeatureTag, FontFeatures, Metrics, Shaping, SwashCache,
    Wrap,
};
use gpui::{
...
use super::font_state::{font_size, with_font_system};
    ...
    fn paint(
        ...
        self.terminal.with_term(|term| {
            ...
            let render_image = if let Some(image) = cached {
                ...
                with_font_system(|font_system, actual_family| {
                    SWASH_CACHE.with_borrow_mut(|swash_cache| {
                        ...
                        let attrs = Attrs::new()
                            .family(cosmic_text::Family::Name(actual_family))
